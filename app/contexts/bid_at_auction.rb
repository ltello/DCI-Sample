...
class BidAtAuction < DCI::Context
  ...
  # Role definitions
    ...
    role :bidder do
      def is_last_bidder?
        auction.last_bidder == player
      end

      def bid
        auction.make_bid(player, settings(:amount))
      end
    end
    ...
    role :auction do
      delegates_to_player :last_bid, :last_bidder, :last_price, :buy_it_now_price, :started?
      ...
      def make_bid(bidder, amount)
        begin
          bids.create!(user: bidder, amount: amount).tap do |bid|
            bid.purchasing? ? CloseAuction[auction: player] : ExtendAuction[auction: player]
          end
      ...
      private_delegates_to_player :bids, :maybe_extend_end_date
    end
